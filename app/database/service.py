    ...
    try:
        ...
        if query_str:
            for field in repository.model.searchable_fields():
                query = query.filter(getattr(repository.model, field).contains(query_str))
        ...
        for filter in filter_spec:
            for filterable_field in repository.model.filterable_fields():
                ...
                if filter["op"] == "eq":
                    query = query.filter(getattr(repository.model, filter["field"]) == filter["value"])
                elif filter["op"] == "ne":
                    query = query.filter(getattr(repository.model, filter["field"]) != filter["value"])
                elif filter["op"] == "lt":
                    query = query.filter(getattr(repository.model, filter["field"]) < filter["value"])
                    ...
                    query = query.filter(getattr(repository.model, filter["field"]).in_(filter["value"]))
                elif filter["op"] == "nin":
                    query = query.filter(getattr(repository.model, filter["field"]).notin_(filter["value"]))
                elif filter["op"] == "startswith":
                    query = query.filter(getattr(repository.model, filter["field"]).startswith(filter["value"]))
                elif filter["op"] == "endswith":
                    query = query.filter(getattr(repository.model, filter["field"]).endswith(filter["value"]))
        ...
        raise ValidationError(
            [ErrorWrapper(InvalidFilterError(msg=str(e)), loc="filter")], model=repository.model
        )
